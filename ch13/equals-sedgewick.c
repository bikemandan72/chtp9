...
Indices partition(int *array, int leftIndex, int rightIndex) {
	...
	int leftEqual = leftIndex - 1, rightEqual = rightIndex;
	...
	for (;;) {
		...
		if (array[left] == pivot) {
			...
			printArray(array, 0, SIZE - 1, left, leftEqual);
		}
		if (array[right] == pivot) {
			...
			printArray(array, 0, SIZE - 1, right, rightEqual);
		}
	}
	...
	for (int i = leftIndex; i < leftEqual; ++i) {
	...
	for (int i = rightIndex - 1; i > rightEqual; --i) {
